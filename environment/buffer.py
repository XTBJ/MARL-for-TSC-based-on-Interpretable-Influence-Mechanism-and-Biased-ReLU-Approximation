'''
@ Summary: Replay buffer for multi-agent RL with parallel rollouts
@ Author: XTBJ
'''
import numpy as np
...
class ReplayBuffer(object):
    def __init__(self, max_steps, num_agents, obs_dims, ac_dims):
        ...
        Inputs:
            ...
            self.obs_buffs: (num_agents, max_steps, obs_dims)
            self.ac_buffs: (num_agents, max_steps, ac_dims)
        """
        ...
        for odim, adim in zip(obs_dims, ac_dims):
            ...
            self.rew_buffs.append(np.zeros((max_steps, adim), dtype=np.float32))
            self.next_obs_buffs.append(np.zeros((max_steps, odim), dtype=np.float32))
            self.done_buffs.append(np.zeros((max_steps, adim), dtype=np.uint8))
            self.wt_buffs.append(np.zeros((max_steps, adim), dtype=np.float32))
        self.filled_i = 0  # index of first empty location in buffer (last index when full)
        self.curr_i = 0  # current index to write to (ovewrite oldest data)
    def __len__(self):
        return self.filled_i

    def push(self, observations, actions, rewards, next_observations, dones, waiting_time):
        ...
        if self.curr_i + nentries > self.max_steps:
            ...
            for agent_i in range(self.num_agents):
                ...
                self.rew_buffs[agent_i] = np.roll(self.rew_buffs[agent_i],
                                                  rollover)
                self.next_obs_buffs[agent_i] = np.roll(self.next_obs_buffs[agent_i],
                                                 rollover, axis=0)
                self.done_buffs[agent_i] = np.roll(self.done_buffs[agent_i],
                                                   rollover)
                self.wt_buffs[agent_i] = np.roll(self.wt_buffs[agent_i],
                                                  rollover)
            self.curr_i = 0
        ...
        for agent_i in range(self.num_agents):
            ...
            self.done_buffs[agent_i][self.curr_i:self.curr_i + nentries] = dones[:, agent_i]
            self.wt_buffs[agent_i][self.curr_i:self.curr_i + nentries] = waiting_time[:, agent_i]
    ...
    def sample(self, N, to_gpu=False, norm_rews=False):
        ...
        if norm_rews:
            ret_rews = [cast((self.rew_buffs[i][inds] -
                             ...
                             (self.rew_buffs[i][:self.filled_i].std() + 1e-5))
        ...
        return ([cast(self.obs_buffs[i][inds]) for i in range(self.num_agents)],
                ...
                [cast(self.wt_buffs[i][inds]) for i in range(self.num_agents)])
